locals {
  ...
  #frontend_ip_configuration_name_lb = var.pip_name
  # locals {
  ...
  # resource_group_name               = var.lbresource_group_name
...
resource "azurerm_lb" "azlb" {
  ...
  dynamic "frontend_ip_configuration" {
    ...
    content {
      ...
      subnet_id                                          = try(frontend_ip_configuration.value.subnet_id, data.azurerm_subnet.lsnet[0].id)
...
resource "azurerm_lb_nat_rule" "azlb_nat_ule" {
  ...
  #backend_port                   = element(var.nat_rule[element(keys(var.remote_port), count.index)], 1)
...
resource "azurerm_lb_rule" "azlb_rule" {
  #  for_each = lenght(var.lb_port) > 0 ? var.lb_port : []
  for_each = var.azlb_rule
  name     = each.key
  #resource_group_name            = var.lb_rule[count.index]["rg_name
  loadbalancer_id                = azurerm_lb.azlb.id
  protocol                       = each.value.protocol
...
# data "azurerm_network_interface" "ext_nic" {
#  count = length(var.exesting_backend_configuration)
#   name                = element(var.exesting_backend_configuration, count.index)
#   resource_group_name = var.nic_resource_group
# }
# locals {
#   backend_pool_association =  try(zipmap(var.network_interface_id, var.ip_configuration_name), {})
# }
